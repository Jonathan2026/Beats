...
// The container that stores our buttons
let beatsContainer = document.getElementById("beats")
let number = 0
...
for(let i = 0; i < wavFiles.length; i++){
    ...
    // Create a button for every sound
    let beatButton = document.createElement("button")
    let sound = wavFiles[i]
    console.log(wavFiles[i])
    beatButton.innerHTML = number + 1;
    ...
    beatButton.addEventListener("click", function(){
        ...
        if(isRecording){
            recordedBeats.push({soundFile: sound, timestamp: performance.now()})
        }

        let audio = new Audio(sound); // Turning our file into a live sound
        audio.currentTime = 0; // Play from the start
        audio.play(); // Plays the beat itself

    })

    beatsContainer.appendChild(beatButton)
    number++ // Change the numbers
}
